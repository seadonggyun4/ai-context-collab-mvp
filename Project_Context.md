...
| 순서 | 단계 | 산출물 |
| --- | --- | --- |
| 1 | 요청 요약 | 사용자 요청을 한 문장으로 정리 |
| 2 | 역할 영향 분류 | planning / publishing / development / qa 영향 여부 판단 |
| 3 | 최신 cycle 확인 | 현재 최신 QA 이후 cycle 디렉토리 확인, 없으면 생성 |
| 4 | 역할별 feature 생성 | 필요한 역할의 `roles/{part}/feature/{cycle}/{next_number}_*.md` 생성 |
| 5 | QA 체크표 생성/갱신 | `roles/qa/feature/` 또는 `roles/qa/qa-results/`에 재검증 기준 기록 |
| 6 | 영향 분석 기록 | 필요 시 `impact-analysis/{date}_*.md` 생성 |
| 7 | 구현 | 문서 게이트 충족 후 코드 수정 |
| 8 | 검증/푸시 | 자동 검증, 필요 시 브라우저 검증, 커밋/푸시 |
