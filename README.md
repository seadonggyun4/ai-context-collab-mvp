...
실행 앱은 결과물이고, `docs/apc-monitoring-mvp/`의 Context·역할·변경 이력 문서와 `docs/organization-standards/`의 회사 공용 규칙은 그 결과물이 어떤 기준과 흐름으로 만들어졌는지 보여주는 협업 엔진입니다.

## 3. Role 구조와 흐름

모든 역할 문서는 `docs/apc-monitoring-mvp/Project_Context.md`를 기준으로 움직입니다. 각 역할은 독립 문서를 갖지만 따로 흩어져 판단하지 않고, 상위 문맥과 서로의 산출물을 참조합니다.

```text
docs/
├── organization-standards/       # 회사 공용 규칙
└── apc-monitoring-mvp/           # 프로젝트별 문서 엔진
    ├── Active_Context.md
    ├── Project_Context.md
    ├── roles/
    ├── change-management/
...
QA 결과 및 변경 영향 분석
├── docs/apc-monitoring-mvp/roles/qa/qa-results/
├── docs/apc-monitoring-mvp/roles/{planning,publishing,development}/feature/{qa-cycle}/
└── docs/apc-monitoring-mvp/impact-analysis/
```
...
모든 신규 변경 요청은 역할별 문서를 만들기 전에 `docs/apc-monitoring-mvp/change-management/`의 Change Manifest에서 시작합니다. Change Manifest는 요청의 현재 상태, 활성 기준, 예상·실제 영향 범위, 테스트 증거와 완료 판정을 한곳에서 연결합니다. 역할별 Feature와 QA Cycle은 상세 산출물을 담당하고, Change Manifest는 요청 전체의 진행 상태를 담당합니다.
...
- 앱 소스: `projects/apc-monitoring-mvp/`
...
- `docs/organization-standards/`: 모든 프로젝트에 적용되는 회사 공용 규칙
- `docs/apc-monitoring-mvp/Project_Context.md`: 프로젝트의 단일 기준 문서
- `docs/apc-monitoring-mvp/Active_Context.md`: 현재 활성 기준 문서 레지스트리
- `docs/apc-monitoring-mvp/change-management/`: 변경 요청, 승인, 증거 및 완료 게이트
- `docs/apc-monitoring-mvp/evaluations/`: Golden Cases와 지침 개선 루프
- `docs/apc-monitoring-mvp/roles/`: 기획·퍼블리싱·개발·QA 기준 및 Feature
- `docs/apc-monitoring-mvp/impact-analysis/`: 변경 영향 분석 이력
- `projects/apc-monitoring-mvp/`: React·FastAPI 실행 프로젝트
