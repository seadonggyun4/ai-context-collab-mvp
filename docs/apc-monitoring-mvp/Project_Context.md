...
| --- | --- |
| status | `ACTIVE` |
| scope | 프로젝트 목적, 사용자, 요구사항, 협업 문서 구조의 최상위 기준 |
...
```text
docs/
├── organization-standards/       # 회사 공용 규칙
└── apc-monitoring-mvp/           # 프로젝트 문서 엔진
    ├── Project_Context.md
    ├── Active_Context.md
    ├── change-management/
    ├── evaluations/
...
모든 신규 변경 요청은 `change-management/templates/change-manifest-template.md` 형식의 Change Manifest를 먼저 생성한다. Change Manifest는 요청별 상태와 완료 근거를 관리하는 제어 문서이며 Project Context나 역할별 기준 문서를 대체하지 않는다. 활성 요청은 `change-management/active/`, 완료 또는 취소된 요청은 `change-management/archive/{year}/`에서 관리한다.

`Project_Context.md`는 프로젝트 내용의 최상위 기준이다. 현재 어떤 문서가 유효한지는 `Active_Context.md`의 레지스트리로 결정한다. AI는 작업 시작 시 Active Context를 읽고 해당 요청에 실제 적용할 문서 목록을 Change Manifest에 선언한다. 최근 파일이라는 이유만으로 기준을 선택하지 않으며, 충돌은 임의 해결하지 않고 사용자에게 보고한다. QA, Impact Analysis, Change Manifest와 Evaluation Result는 기준 문서가 아니라 증거와 이력이다.

변경 요청은 `REQUESTED → CONTEXT_CONFIRMED → PLANNED → DEVELOPMENT_READY → IMPLEMENTED → QA_COMPLETED → SELF_REVIEWED → COMPLETED` 순서로 진행한다. 각 상태의 진입 조건과 필수 증거는 `change-management/README.md`를 따른다. AI는 상태를 건너뛰거나 실행하지 않은 테스트를 완료 근거로 사용할 수 없다.

상태와 별도로 단일 AI의 현재 역할은 `CONTEXT_READER → PLANNER → IMPLEMENTER → TESTER → SELF_REVIEWER → REPORTER` Phase로 관리한다. State는 요청의 성숙도, Phase는 AI의 현재 책임을 나타낸다. Phase가 바뀌기 전에는 해당 Change Manifest에 `Phase Transition` 선언을 기록하며, 요구사항 변경이나 검증 실패가 있으면 전환 이력을 삭제하지 않고 책임 Phase로 되돌아간다.

Self-Review는 독립 검증으로 표현하지 않는다. 원 요청, Active Context, 실제 diff와 Verification Evidence를 다시 읽고 의도와 실제 확인 결과, 반례, confidence, 미검증 사항을 분리한다. 고위험 변경은 `change-management/approval-policy.md`에 따라 사용자 승인 전 실제 적용을 중단한다.
...
사용자가 배포 화면, 문서, 동작을 보고 직접 수정 요청을 하는 경우도 동일한 문서 엔진을 따른다. AI는 수정 요청을 받으면 바로 구현하지 않고 먼저 해당 요청이 `planning`, `publishing`, `development`, `qa` 중 어디에 속하는지 분류한다. 그 다음 최신 QA cycle 또는 다음 numbered cycle의 `roles/{part}/feature/{cycle}/` 디렉토리를 만들고, 각 역할에 필요한 feature 문서를 다음 순번으로 생성한다. 코드 수정은 planning/publishing/development/qa 문서 산출과 영향 분석 기록 이후에 진행한다.
...
| `04_after_fourth_qa/` | `fourth_qa_check.md` 이후 또는 사용자 수정 요청 발생 시 | 네 번째 QA 이후 변경 요청을 역할별 feature와 QA 체크표로 전환 | `roles/development/feature/04_after_fourth_qa/18_change_request_implementation_gate.md` |
...
사용자가 “수정해줘”, “이 부분 이상해”, “제거해줘”, “문구 바꿔줘”, “레이아웃 바꿔줘”처럼 변경을 요청하면 AI는 다음 순서를 반드시 따른다.

| 순서 | 단계 | 산출물 |
| --- | --- | --- |
| 1 | Change Manifest 생성 | 요청 원문, 요약, 포함/제외 범위를 `REQUESTED`로 기록 |
| 2 | Context 확인 | 활성 기준 문서와 충돌 여부를 기록하고 `CONTEXT_CONFIRMED`로 전환 |
| 3 | 역할 영향 분류 | planning / publishing / development / qa 영향과 예상 파일을 기록 |
| 4 | 최신 cycle 확인 | 현재 최신 QA 이후 cycle 디렉토리 확인, 없으면 생성 |
| 5 | 역할별 feature 생성 | 필요한 역할의 `roles/{part}/feature/{cycle}/{next_number}_*.md` 생성 |
| 6 | QA·영향 분석 계획 | QA 체크표, 테스트 계획, `impact-analysis/` 대상을 연결 |
| 7 | Development Ready 확인 | 문서·범위·위험·테스트 게이트 충족 근거 기록 |
| 8 | 구현 | `DEVELOPMENT_READY` 이후 계획 범위 안에서 수정 |
| 9 | 검증과 자체 검토 | 실제 변경 대조, 실행 증거, 반례 검토 기록 |
| 10 | 완료 | 완료 조건을 모두 충족한 경우에만 `COMPLETED` 선언 |

역할 영향 분류 기준은 다음과 같다.

| 역할 | 분류 기준 |
| --- | --- |
| Planning | 사용자 흐름, 메뉴, 탭, CTA, 필터, 예외 처리, 문구 정책, 완료 기준이 바뀌는가 |
| Publishing | 레이아웃, 폭, 간격, 색상, 컴포넌트, 상태 표현, 반응형, 접근성이 바뀌는가 |
| Development | API, 상태관리, 타입, 데이터 변환, 이벤트 핸들링, 테스트 코드가 바뀌는가 |
| QA | 변경 결과를 어떤 체크표와 시나리오로 재검증해야 하는가 |

수정 요청이 단순해 보이더라도 문서 히스토리를 생략하지 않는다. 변경이 코드 한 줄이어도 사용자 흐름이나 화면 표현에 영향을 주면 해당 역할 feature 문서를 생성한다.
...
`projects/apc-monitoring-mvp/`은 문서 기준이 실제 구현물로 전환되는 영역이다. 문서 산출물과 실행 코드는 분리하되, 구현은 항상 Project Context와 직군별 문서를 참조한다.
...
| viewer | 조회자 역할. 상태와 요약 정보는 볼 수 있으나 origin/refined 상세 경로는 제한된다. |
| operator | 운영자 역할. 품질 이슈 상세와 제한 경로를 확인할 수 있다. MVP에서는 DB 저장이 필요한 조치 작성은 제공하지 않는다. |
| admin | 관리자 역할. 권한 필요 정보를 확인할 수 있다. MVP에서는 기준 수정 같은 DB 저장 기능은 제공하지 않는다. |
...
| 2026-07-09 | 실제 구현을 위한 `projects/apc-monitoring-mvp/` 작업공간 생성 | 문서 산출물과 실행 코드 분리, Phase 0 완료 |
...
| 2026-07-09 | 네 번째 QA 체크 완료 | QA3-001 재검증 통과, 자동/브라우저 검증 완료, QA 실패 기반 신규 미해결 항목은 없음 |
| 2026-07-09 | 사용자 수정 요청 처리 규칙 보강 | 수정 요청이 들어오면 역할 영향 분류 후 최신 numbered cycle에 planning/publishing/development/QA 문서와 impact-analysis를 먼저 생성하도록 문서 엔진 기준 추가 |
| 2026-07-09 | 중복 flow UI 제거 및 수신 현황 전용 matrix filter 개선 | `JADX 메뉴 흐름 시나리오` 제거, Matrix 선택 조건을 수신 현황 탭 내부 selectbox로 제한, 다섯 번째 QA 통과 |
| 2026-07-09 | 기능 없는 전역 컨트롤 제거 | 기준일/APC/품목/입고·선별/상태 전역 필터와 헤더 우측 중복 CTA 제거, 여섯 번째 QA 통과 |
| 2026-07-09 | DB 없는 mock write/settings 기능 제거 | 운영 조치 내역, 모니터링 기준 설정, 품질 이슈 조치 form, action/rule API 제거. MVP를 읽기 중심 문서 기반 AI 엔진 시연으로 축소하고 일곱 번째 QA로 검증 |
| 2026-07-14 | 갱신일/조회일 동적 날짜 보정 | 헤더 마지막 갱신, frontend 기본 조회일, backend fixture 응답 날짜가 KST 오늘 기준으로 표시되도록 개선하고 여덟 번째 QA로 검증 |
| 2026-07-22 | 공용 규칙·프로젝트 문서 엔진·실행 코드를 각각 `docs/organization-standards/`, `docs/apc-monitoring-mvp/`, `projects/apc-monitoring-mvp/`로 재배치 | 다중 프로젝트 확장과 비개발자용 문서 관리 UI를 위한 저장소 경계 확립 |
