...
| Phase 2 ◐ | Backend 기반·Render 초기 배포 | Python 3.12/FastAPI scaffold, settings, health, PostgreSQL, SQLAlchemy/Alembic, Project/Document read API, Git read adapter, zero-cost root `render.yaml` | 코드·PostgreSQL·Blueprint schema와 `imugi` workspace validation 완료; 실제 free deploy smoke 대기 |
